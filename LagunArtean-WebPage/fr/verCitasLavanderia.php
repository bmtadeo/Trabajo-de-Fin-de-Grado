...
<head>
    <meta charset="UTF-8">
    <title>Voir le site Laundry Dating</title>
...
<section>
    <nav>
        <ul>
            ...
            <li><a class="active" href="verCitasLavanderia.php">Voir le site Laundry Dating</a></li>
...
<section>
    ...
    if(isset($_SESSION["usu_id"])){
        if ($result->num_rows > 0) {
            echo "<p><b>Les rendez-vous de blanchisserie :</b></p>";
            echo "<table id='citasLavanderia'><tr><th>Date de livraisona</th><th>Date de collecte</th><th>Livré à</th><th>Collection</th></tr>";
            while($row = $result->fetch_assoc()) {
                if (($row["recogida"]==0)&& ($row["fechaRecogida"]=="")){
                    $recogida ="Pas encore de date de prise en charge";
                }else if (($row["recogida"]==0) && ($row["fechaRecogida"]!="")){
                    $recogida ="Veuillez le retirer le jour:" .$row["fechaRecogida"];
                }
                if ($row["entregada"]==0){
                    $entregada ="Veuillez le tourner dans la journée: ".$row["fecha"];
            ...
            echo "<p>Vous n'avez pas de rendez-vous pour la blanchisserie.</p>";
        }
    }
    ?>
    <form id="borrarCitaLavanderia" method="post" action="borrarCitaLavanderia.php">
        <h3>Supprimer le rendez-vous de blanchisserie</h3>
