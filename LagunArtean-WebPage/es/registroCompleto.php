...
<section>
    <h2>Con el siguiente usuario y contraseña puedes iniciar sesion en el sistema <br>
        Si lo deseas puedes cambiar la contraseña en cualquier momento desde el menu de inicio.</h2>
    <h3>Usuario: <?php echo $_SESSION["usuRegistro"]; ?></h3>
    <h3>Contraseña: <?php echo $_SESSION["contraRegistro"]; ?> </h3>
    <?php echo printQRCode($qr); ?>
    <?php
    function printQRCode($url, $size = 100) {
        return '<img src="http://chart.apis.google.com/chart?chs=150x150&cht=qr&chl=' . urlencode($url) . '" />';
    }
    ?>
</section>
</body>
